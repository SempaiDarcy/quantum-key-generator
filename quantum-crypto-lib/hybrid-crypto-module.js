// include: shell.js
// Объект Module: наш интерфейс для взаимодействия с внешним миром.
// Мы импортируем и экспортируем значения через него. Существует несколько способов использования Module:
// 1. Не определён. Мы создаём его здесь.
// 2. Передаётся как параметр функции, например: function(Module) { ..сгенерированный код.. }
// 3. Добавлен до запуска, например: var Module = {}; ..сгенерированный код..
// 4. Внешний скрипт определяет переменную var Module.
// Нужно проверить, существует ли уже Module (например, случай 3 выше).
// Замены будут подставлены фактическим кодом на более позднем этапе сборки,
// чтобы компилятор Closure не переименовывал их (например, случай 4).
// Обрати внимание: если вы хотите использовать Closure Compiler,
// и при этом использовать Module после сгенерированного кода,
// нужно заранее определить   var Module = {};
// Тогда этот объект будет использоваться в коде, и вы сможете
// продолжать работать с Module и после выполнения кода.
var Module = typeof Module != 'undefined' ? Module : {};

// --pre-js файлы вставляются после кода интеграции Module, чтобы они могли
// обращаться к Module (если захотят; они также могут его определить)


// Иногда уже существует объект Module с определёнными свойствами,
// предназначенными для переопределения стандартной функциональности модуля.
// Здесь мы собираем эти свойства и применяем их снова _после_ того,
// как настроим значения по умолчанию для текущего окружения,
// чтобы избежать необходимости излишне защищать и проверять их во время инициализации.

var moduleOverrides = Object.assign({}, Module);

var arguments_ = [];
var thisProgram = './this.program';
var quit_ = (status, toThrow) => {
    throw toThrow;
};

// Определяем, в каком среде выполнения мы находимся.
// Вы можете настроить это, указав параметр ENVIRONMENT во время компиляции (см. settings.js).

var ENVIRONMENT_IS_WEB = false;
var ENVIRONMENT_IS_WORKER = false;
var ENVIRONMENT_IS_NODE = true;
var ENVIRONMENT_IS_SHELL = false;

if (Module['ENVIRONMENT']) {
    throw new Error('Module.ENVIRONMENT устарел. Чтобы указать среду выполнения, используйте опцию компиляции ENVIRONMENT (например, -sENVIRONMENT=web или -sENVIRONMENT=node).');
}

// Если `scriptDirectory` не пустой, то в конце должен быть символ `/`
var scriptDirectory = '';

function locateFile(path) {
    if (Module['locateFile']) {
        return Module['locateFile'](path, scriptDirectory);
    }
    return scriptDirectory + path;
}

// Перехватчики (hooks), которые реализуются по-разному в разных средах выполнения.
var read_,
    readAsync,
    readBinary;

if (ENVIRONMENT_IS_NODE) {
    if (typeof process == 'undefined' || !process.release || process.release.name !== 'node') throw new Error('not compiled for this environment (did you build to HTML and try to run it not on the web, or set ENVIRONMENT to something - like node - and run it someplace else - like on the web?)');

    var nodeVersion = process.versions.node;
    var numericVersion = nodeVersion.split('.').slice(0, 3);
    numericVersion = (numericVersion[0] * 10000) + (numericVersion[1] * 100) + (numericVersion[2].split('-')[0] * 1);
    var minVersion = 160000;
    if (numericVersion < 160000) {
        throw new Error('This emscripten-generated code requires node v16.0.0 (detected v' + nodeVersion + ')');
    }

    // В ESM-модуле вызов `require()` ничего не делает, вместо этого нужно использовать
    // `createRequire()`, чтобы создать функцию `require()`.
    // Это требуется только для сборок под несколько сред,
    // при использовании `-sENVIRONMENT=node` будет сгенерирован статический импорт.
    // TODO: Заменить все вызовы `require()` на `import()`?
    // Обычно эти модули используются в Node.js. Загружаем их сразу,
    // чтобы избежать сложности отложенной загрузки.

    var fs = require('fs');
    var nodePath = require('path');

    if (ENVIRONMENT_IS_WORKER) {
        scriptDirectory = nodePath.dirname(scriptDirectory) + '/';
    } else {
        scriptDirectory = __dirname + '/';
    }

    // подключение: node_shell_read.js
    read_ = (filename, binary) => {
        // Нужно заново обернуть строки, начинающиеся с `file://`, в объекты URL.
        // В этом случае нормализация не требуется, путь уже должен быть абсолютным.
        filename = isFileURI(filename) ? new URL(filename) : nodePath.normalize(filename);
        return fs.readFileSync(filename, binary ? undefined : 'utf8');
    };

    readBinary = (filename) => {
        var ret = read_(filename, true);
        if (!ret.buffer) {
            ret = new Uint8Array(ret);
        }
        assert(ret.buffer);
        return ret;
    };

    readAsync = (filename, onload, onerror, binary = true) => {
        // Смотрите комментарий в функции `read_`.
        filename = isFileURI(filename) ? new URL(filename) : nodePath.normalize(filename);
        fs.readFile(filename, binary ? undefined : 'utf8', (err, data) => {
            if (err) onerror(err);
            else onload(binary ? data.buffer : data);
        });
    };
    // end include: node_shell_read.js
    if (!Module['thisProgram'] && process.argv.length > 1) {
        thisProgram = process.argv[1].replace(/\\/g, '/');
    }

    arguments_ = process.argv.slice(2);

    if (typeof module != 'undefined') {
        module['exports'] = Module;
    }

    process.on('uncaughtException', (ex) => {
        // подавлять исключения ExitStatus, чтобы они не отображали сообщение об ошибке
        if (ex !== 'unwind' && !(ex instanceof ExitStatus) && !(ex.context instanceof ExitStatus)) {
            throw ex;
        }
    });

    quit_ = (status, toThrow) => {
        process.exitCode = status;
        throw toThrow;
    };

} else if (ENVIRONMENT_IS_SHELL) {

    if ((typeof process == 'object' && typeof require === 'function') || typeof window == 'object' || typeof importScripts == 'function')
        throw new Error('Не скомпилировано для этой среды (вы собрали под HTML и пытаетесь запускать не в вебе, или задали ENVIRONMENT как node и запускаете в другой среде, например в браузере)');

} else

// Обратите внимание, что сюда входят воркеры Node.js, когда это актуально (если включён pthreads).
// Воркеры Node.js определяются как сочетание ENVIRONMENT_IS_WORKER и ENVIRONMENT_IS_NODE.
{
    throw new Error('Ошибка определения среды выполнения');
}

var out = Module['print'] || console.log.bind(console);
var err = Module['printErr'] || console.error.bind(console);

// Объединяем обратно переопределения
Object.assign(Module, moduleOverrides);
// Освобождаем иерархию объектов из переопределений, чтобы сборщик мусора освободил данные
moduleOverrides = null;
checkIncomingModuleAPI();

// Генерируем код для обработки ожидаемых значений в объекте Module.
// Это применяет Module.x к локальной переменной x.
// Это даёт два преимущества: во-первых, мы добавляем код только если свойство ожидается,
// во-вторых, локальная переменная может быть сжата при минификации.

if (Module['arguments']) arguments_ = Module['arguments'];
legacyModuleProp('arguments', 'arguments_');

if (Module['thisProgram']) thisProgram = Module['thisProgram'];
legacyModuleProp('thisProgram', 'thisProgram');

if (Module['quit']) quit_ = Module['quit'];
legacyModuleProp('quit', 'quit_');

// Выполняем утверждения после настройки функций вывода, чтобы при ошибке можно было напечатать сообщение
// Утверждения для удалённых API свойств Module
assert(typeof Module['memoryInitializerPrefixURL'] == 'undefined', 'Параметр Module.memoryInitializerPrefixURL был удалён, используйте Module.locateFile вместо него');
assert(typeof Module['pthreadMainPrefixURL'] == 'undefined', 'Параметр Module.pthreadMainPrefixURL был удалён, используйте Module.locateFile вместо него');
assert(typeof Module['cdInitializerPrefixURL'] == 'undefined', 'Параметр Module.cdInitializerPrefixURL был удалён, используйте Module.locateFile вместо него');
assert(typeof Module['filePackagePrefixURL'] == 'undefined', 'Параметр Module.filePackagePrefixURL был удалён, используйте Module.locateFile вместо него');
assert(typeof Module['read'] == 'undefined', 'Параметр Module.read был удалён (измените функцию read_ в JS)');
assert(typeof Module['readAsync'] == 'undefined', 'Параметр Module.readAsync был удалён (измените функцию readAsync в JS)');
assert(typeof Module['readBinary'] == 'undefined', 'Параметр Module.readBinary был удалён (измените функцию readBinary в JS)');
assert(typeof Module['setWindowTitle'] == 'undefined', 'Параметр Module.setWindowTitle был удалён (используйте emscripten_set_window_title в JS)');
assert(typeof Module['TOTAL_MEMORY'] == 'undefined', 'Module.TOTAL_MEMORY переименован в Module.INITIAL_MEMORY');
legacyModuleProp('asm', 'wasmExports');
legacyModuleProp('read', 'read_');
legacyModuleProp('readAsync', 'readAsync');
legacyModuleProp('readBinary', 'readBinary');
legacyModuleProp('setWindowTitle', 'setWindowTitle');

// Сообщения о том, что файловые системы больше не подключаются по умолчанию
var IDBFS = 'IDBFS больше не включён по умолчанию; собирайте с -lidbfs.js';
var PROXYFS = 'PROXYFS больше не включён по умолчанию; собирайте с -lproxyfs.js';
var WORKERFS = 'WORKERFS больше не включён по умолчанию; собирайте с -lworkerfs.js';
var FETCHFS = 'FETCHFS больше не включён по умолчанию; собирайте с -lfetchfs.js';
var ICASEFS = 'ICASEFS больше не включён по умолчанию; собирайте с -licasefs.js';
var JSFILEFS = 'JSFILEFS больше не включён по умолчанию; собирайте с -ljsfilefs.js';
var OPFS = 'OPFS больше не включён по умолчанию; собирайте с -lopfs.js';
var NODEFS = 'NODEFS больше не включён по умолчанию; собирайте с -lnodefs.js';

// Проверки среды
assert(!ENVIRONMENT_IS_WEB, 'Обнаружена среда web, но не включена при сборке. Добавьте `web` в `-sENVIRONMENT` для активации.');
assert(!ENVIRONMENT_IS_WORKER, 'Обнаружена среда worker, но не включена при сборке. Добавьте `worker` в `-sENVIRONMENT` для активации.');
assert(!ENVIRONMENT_IS_SHELL, 'Обнаружена среда shell, но не включена при сборке. Добавьте `shell` в `-sENVIRONMENT` для активации.');


// end include: shell.js

// include: preamble.js
// === Библиотечный пролог ===

// Документацию по публичным API, определённым в этом файле, нужно обновлять в:
// site/source/docs/api_reference/preamble.js.rst
// Локальная версия документации доступна по адресу:
// site/build/text/docs/api_reference/preamble.js.txt
// Также можно собрать документацию локально в HTML или других форматах
// Онлайн-версия документации (возможно другой версии Emscripten):
// http://kripken.github.io/emscripten-site/docs/api_reference/preamble.js.html

var wasmBinary;
if (Module['wasmBinary']) wasmBinary = Module['wasmBinary'];
legacyModuleProp('wasmBinary', 'wasmBinary');

if (typeof WebAssembly != 'object') {
    err('Не обнаружена нативная поддержка WebAssembly');
}

// include: base64Utils.js
// Преобразует строку base64 в массив байтов (Uint8Array)
function intArrayFromBase64(s) {
    if (typeof ENVIRONMENT_IS_NODE != 'undefined' && ENVIRONMENT_IS_NODE) {
        var buf = Buffer.from(s, 'base64');
        return new Uint8Array(buf.buffer, buf.byteOffset, buf.length);
    }

    var decoded = atob(s);
    var bytes = new Uint8Array(decoded.length);
    for (var i = 0; i < decoded.length; ++i) {
        bytes[i] = decoded.charCodeAt(i);
    }
    return bytes;
}

// Если имя файла — data URI в base64, парсит и возвращает данные (Buffer в node, Uint8Array в других средах).
// Если это не data URI, возвращает undefined.
function tryParseAsDataURI(filename) {
    if (!isDataURI(filename)) {
        return;
    }

    return intArrayFromBase64(filename.slice(dataURIPrefix.length));
}

// end include: base64Utils.js
// Глобальные переменные Wasm

var wasmMemory;

// ========================================
// Основные элементы рантайма
// ========================================

// Указывает, завершается ли приложение. После этого код не должен выполняться.
// Устанавливается в exit() и abort()
var ABORT = false;

// Устанавливается в exit() и abort(). Передаётся в обработчик 'onExit'.
// Также используется как код возврата процесса в shell-средах,
// но только если noExitRuntime = false.
var EXITSTATUS;

// В режиме STRICT мы определяем assert() только если ASSERTIONS установлены,
// то есть в релизных сборках она вообще не определяется.
// Это соответствует поведению MINIMAL_RUNTIME.
// TODO(sbc): Сделать это поведение стандартным даже без STRICT.
/** @type {function(*, string=)} */
function assert(condition, text) {
    if (!condition) {
        abort('Assertion failed' + (text ? ': ' + text : ''));
    }
}

// Раньше по умолчанию включались malloc/free. Покажем понятную ошибку,
// если сборка с ASSERTIONS.

// Управление памятью

var HEAP,
    /** @type {!Int8Array} */
    HEAP8,
    /** @type {!Uint8Array} */
    HEAPU8,
    /** @type {!Int16Array} */
    HEAP16,
    /** @type {!Uint16Array} */
    HEAPU16,
    /** @type {!Int32Array} */
    HEAP32,
    /** @type {!Uint32Array} */
    HEAPU32,
    /** @type {!Float32Array} */
    HEAPF32,
    /** @type {!Float64Array} */
    HEAPF64;

// include: runtime_shared.js
// Обновляет представления памяти (HEAP8, HEAP16 и др.) при изменении буфера wasmMemory

function updateMemoryViews() {
    var b = wasmMemory.buffer;
    Module['HEAP8'] = HEAP8 = new Int8Array(b);
    Module['HEAP16'] = HEAP16 = new Int16Array(b);
    Module['HEAPU8'] = HEAPU8 = new Uint8Array(b);
    Module['HEAPU16'] = HEAPU16 = new Uint16Array(b);
    Module['HEAP32'] = HEAP32 = new Int32Array(b);
    Module['HEAPU32'] = HEAPU32 = new Uint32Array(b);
    Module['HEAPF32'] = HEAPF32 = new Float32Array(b);
    Module['HEAPF64'] = HEAPF64 = new Float64Array(b);
}

// end include: runtime_shared.js
assert(!Module['STACK_SIZE'], 'STACK_SIZE больше нельзя задавать во время выполнения. Используйте -sSTACK_SIZE при линковке');

assert(
    typeof Int32Array != 'undefined' &&
    typeof Float64Array !== 'undefined' &&
    Int32Array.prototype.subarray != undefined &&
    Int32Array.prototype.set != undefined,
    'JS-движок не поддерживает полноценные типизированные массивы'
);

// Если память определена в wasm, пользователь не может задавать её или INITIAL_MEMORY
assert(!Module['wasmMemory'], 'Обнаружено использование `wasmMemory`. Используйте -sIMPORTED_MEMORY для внешнего определения wasmMemory');
assert(!Module['INITIAL_MEMORY'], 'Обнаружено задание INITIAL_MEMORY в рантайме. Используйте -sIMPORTED_MEMORY для динамического определения памяти');

// include: runtime_stack_check.js
// Инициализация контрольных значений стека. Вызывается при старте main() и в каждом pthread-е.
function writeStackCookie() {
    var max = _emscripten_stack_get_end();
    assert((max & 3) == 0);
    // Если стек оканчивается по адресу 0, записываем cookies на 4 байта дальше,
    // чтобы не пересекаться с SAFE_HEAP и ASAN.
    if (max == 0) {
        max += 4;
    }
    // Стек растёт вниз к _emscripten_stack_get_end.
    // Записываем контрольные значения в последние два слова стека,
    // чтобы отследить их перезапись.
    HEAPU32[((max) >> 2)] = 0x02135467;
    HEAPU32[(((max) + (4)) >> 2)] = 0x89BACDFE;
    // Также проверяем глобальный адрес 0
    HEAPU32[((0) >> 2)] = 1668509029;
}

// Проверка целостности стека по контрольным значениям
function checkStackCookie() {
    if (ABORT) return;
    var max = _emscripten_stack_get_end();
    if (max == 0) {
        max += 4;
    }
    var cookie1 = HEAPU32[((max) >> 2)];
    var cookie2 = HEAPU32[(((max) + (4)) >> 2)];
    if (cookie1 != 0x02135467 || cookie2 != 0x89BACDFE) {
        abort(`Переполнение стека! Контрольное значение было перезаписано по адресу ${ptrToString(max)}, ожидались слова 0x89BACDFE и 0x2135467, но получены ${ptrToString(cookie2)} ${ptrToString(cookie1)}`);
    }
    if (HEAPU32[((0) >> 2)] != 0x63736d65 /* 'emsc' */) {
        abort('Ошибка выполнения: приложение повредило область памяти (адрес ноль)!');
    }
}
// end include: runtime_stack_check.js
// include: runtime_assertions.js
// Проверка порядка байт (Endianness)
(function () {
    var h16 = new Int16Array(1);
    var h8 = new Int8Array(h16.buffer);
    h16[0] = 0x6373;
    if (h8[0] !== 0x73 || h8[1] !== 0x63)
        throw 'Ошибка выполнения: ожидалась система little-endian! (Запустите с -sSUPPORT_BIG_ENDIAN, чтобы игнорировать)';
})();

// end include: runtime_assertions.js

// Колбэки, вызываемые в разные моменты жизненного цикла
var __ATPRERUN__ = []; // функции, вызываемые до инициализации рантайма
var __ATINIT__ = []; // функции, вызываемые при запуске
var __ATEXIT__ = []; // функции, вызываемые при завершении
var __ATPOSTRUN__ = []; // функции, вызываемые после main()

var runtimeInitialized = false;

// Выполняет preRun() колбэки
function preRun() {
    if (Module['preRun']) {
        if (typeof Module['preRun'] == 'function') Module['preRun'] = [Module['preRun']];
        while (Module['preRun'].length) {
            addOnPreRun(Module['preRun'].shift());
        }
    }
    callRuntimeCallbacks(__ATPRERUN__);
}

// Инициализация рантайма
function initRuntime() {
    assert(!runtimeInitialized);
    runtimeInitialized = true;

    checkStackCookie();

    callRuntimeCallbacks(__ATINIT__);
}

// Завершение рантайма
function postRun() {
    checkStackCookie();

    if (Module['postRun']) {
        if (typeof Module['postRun'] == 'function') Module['postRun'] = [Module['postRun']];
        while (Module['postRun'].length) {
            addOnPostRun(Module['postRun'].shift());
        }
    }

    callRuntimeCallbacks(__ATPOSTRUN__);
}

function addOnPreRun(cb) {
    __ATPRERUN__.unshift(cb);
}

function addOnInit(cb) {
    __ATINIT__.unshift(cb);
}

function addOnExit(cb) {
}

function addOnPostRun(cb) {
    __ATPOSTRUN__.unshift(cb);
}

// include: runtime_math.js
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/imul
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/fround
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/clz32
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/trunc

assert(Math.imul, 'Этот браузер не поддерживает Math.imul(), соберите с LEGACY_VM_SUPPORT или POLYFILL_OLD_MATH_FUNCTIONS для добавления полифила');
assert(Math.fround, 'Этот браузер не поддерживает Math.fround(), соберите с LEGACY_VM_SUPPORT или POLYFILL_OLD_MATH_FUNCTIONS для добавления полифила');
assert(Math.clz32, 'Этот браузер не поддерживает Math.clz32(), соберите с LEGACY_VM_SUPPORT или POLYFILL_OLD_MATH_FUNCTIONS для добавления полифила');
assert(Math.trunc, 'Этот браузер не поддерживает Math.trunc(), соберите с LEGACY_VM_SUPPORT или POLYFILL_OLD_MATH_FUNCTIONS для добавления полифила');
// end include: runtime_math.js

// Счётчик зависимостей для вызова run(). Если требуется
// асинхронная работа перед запуском, нужно увеличить этот счётчик.
// Увеличение должно происходить, например, в Module.preRun
// (используется emcc для предзагрузки файлов).
// Можно добавлять зависимости в preRun, даже если он выполняется прямо перед run -
// запуск будет отложен, пока зависимости не будут удовлетворены.
var runDependencies = 0;
var runDependencyWatcher = null;
var dependenciesFulfilled = null; // Переопределяется для других действий после выполнения зависимостей
var runDependencyTracking = {};

function getUniqueRunDependency(id) {
    var orig = id;
    while (1) {
        if (!runDependencyTracking[id]) return id;
        id = orig + Math.random();
    }
}

function addRunDependency(id) {
    runDependencies++;

    Module['monitorRunDependencies']?.(runDependencies);

    if (id) {
        assert(!runDependencyTracking[id]);
        runDependencyTracking[id] = 1;
        if (runDependencyWatcher === null && typeof setInterval != 'undefined') {
            // Каждые несколько секунд проверять наличие зависимостей
            runDependencyWatcher = setInterval(() => {
                if (ABORT) {
                    clearInterval(runDependencyWatcher);
                    runDependencyWatcher = null;
                    return;
                }
                var shown = false;
                for (var dep in runDependencyTracking) {
                    if (!shown) {
                        shown = true;
                        err('Ожидание зависимостей при запуске:');
                    }
                    err(`Зависимость: ${dep}`);
                }
                if (shown) {
                    err('(конец списка)');
                }
            }, 10000);
        }
    } else {
        err('предупреждение: зависимость при запуске добавлена без ID');
    }
}

function removeRunDependency(id) {
    runDependencies--;

    Module['monitorRunDependencies']?.(runDependencies);

    if (id) {
        assert(runDependencyTracking[id]);
        delete runDependencyTracking[id];
    } else {
        err('предупреждение: зависимость при запуске удалена без ID');
    }
    if (runDependencies == 0) {
        if (runDependencyWatcher !== null) {
            clearInterval(runDependencyWatcher);
            runDependencyWatcher = null;
        }
        if (dependenciesFulfilled) {
            var callback = dependenciesFulfilled;
            dependenciesFulfilled = null;
            callback(); // может добавить ещё dependenciesFulfilled
        }
    }
}

/** @param {string|number=} what */
function abort(what) {
    Module['onAbort']?.(what);

    what = 'Прервано(' + what + ')';
    // TODO(sbc): Возможно, стоит убрать печать и оставить её тому,
    // кто перехватывает исключение?
    err(what);

    ABORT = true;
    EXITSTATUS = 1;

    // Используем RuntimeError wasm, чтобы JS-ошибка не считалась "чужим"
    // исключением, которое может вызвать выполнение деструкторов.
    // FIXME: Этот подход не работает в Wasm EH, так как он не считает все
    // RuntimeError капканами. Нужен API, который позволит это сделать в спецификации wasm.

    /** @suppress {checkTypes} */
    var e = new WebAssembly.RuntimeError(what);

    // Бросаем ошибку независимо от MODULARIZE, так как abort вызывается
    // не только в момент создания модуля, и ожидается исключение.
    throw e;
}

// include: memoryprofiler.js
// end include: memoryprofiler.js

// Показ ошибок при вызовах FS, если он не был включён
var FS = {
    error() {
        abort('Поддержка файловой системы (FS) не была включена. Проблема в том, что вы используете файлы из JS, но они не используются из C/C++, поэтому поддержка FS не была добавлена автоматически. Чтобы принудительно включить поддержку файловой системы, соберите с -sFORCE_FILESYSTEM');
    },
    init() {
        FS.error()
    },
    createDataFile() {
        FS.error()
    },
    createPreloadedFile() {
        FS.error()
    },
    createLazyFile() {
        FS.error()
    },
    open() {
        FS.error()
    },
    mkdev() {
        FS.error()
    },
    registerDevice() {
        FS.error()
    },
    analyzePath() {
        FS.error()
    },
    ErrnoError() {
        FS.error()
    },
};
Module['FS_createDataFile'] = FS.createDataFile;
Module['FS_createPreloadedFile'] = FS.createPreloadedFile;
// include: URIUtils.js
// Префикс data URI, используемый при опции SINGLE_FILE и схожих настройках.
var dataURIPrefix = 'data:application/octet-stream;base64,';

/**
 * Проверяет, является ли имя файла base64 data URI.
 * @noinline
 */
var isDataURI = (filename) => filename.startsWith(dataURIPrefix);

/**
 * Проверяет, передаётся ли файл через протокол file:// (а не http/https)
 * @noinline
 */
var isFileURI = (filename) => filename.startsWith('file://');

// end include: URIUtils.js

// Создаёт обёртку для экспорта wasm-функции
function createExportWrapper(name, nargs) {
    return (...args) => {
        assert(runtimeInitialized, `Нативная функция \`${name}\` вызвана до инициализации рантайма`);
        var f = wasmExports[name];
        assert(f, `Экспортированная нативная функция \`${name}\` не найдена`);
        // Проверяем только, что аргументов не слишком много.
        // Недостающие аргументы будут автоматически заполнены нулями.
        assert(args.length <= nargs, `Нативная функция \`${name}\` вызвана с ${args.length} аргументами, но ожидает ${nargs}`);
        return f(...args);
    };
}

// include: runtime_exceptions.js
// end include: runtime_exceptions.js

var wasmBinaryFile;
wasmBinaryFile = 'data:application/octet-stream;base64,';

function getBinarySync(file) {
    if (file == wasmBinaryFile && wasmBinary) {
        return new Uint8Array(wasmBinary);
    }
    var binary = tryParseAsDataURI(file);
    if (binary) {
        return binary;
    }
    if (readBinary) {
        return readBinary(file);
    }
    throw 'Не удалось синхронно получить wasm: вы можете вручную загрузить его в Module["wasmBinary"], или emcc.py сделает это при генерации HTML (но не JS)';
}

function getBinaryPromise(binaryFile) {
    // В противном случае getBinarySync должен получить его синхронно
    return Promise.resolve().then(() => getBinarySync(binaryFile));
}

function instantiateSync(file, info) {
    var module;
    var binary = getBinarySync(file);
    module = new WebAssembly.Module(binary);
    var instance = new WebAssembly.Instance(module, info);
    return [instance, module];
}

// Создаёт экземпляр wasm.
// Получает импорты wasm, возвращает экспорты.
function createWasm() {
    // Подготовка импортов
    var info = {
        'env': wasmImports,
        'wasi_snapshot_preview1': wasmImports,
    };
    // Загружаем wasm-модуль и создаём экземпляр, используя нативную поддержку JS-движка.
    // Обработка созданного экземпляра: получаем его экспорты и выполняем настройку.
    /** @param {WebAssembly.Module=} module*/
    function receiveInstance(instance, module) {
        wasmExports = instance.exports;

        wasmMemory = wasmExports['memory'];

        assert(wasmMemory, 'memory не найден в экспортах wasm');
        updateMemoryViews();

        addOnInit(wasmExports['__wasm_call_ctors']);

        removeRunDependency('wasm-instantiate');
        return wasmExports;
    }

    // Ждём пула pthread (если есть)
    addRunDependency('wasm-instantiate');

    // Предпочитаем потоковую инициализацию, если возможно.

    // Пользовательские shell-страницы могут определить свой Module.instantiateWasm = function(imports, successCallback)
    // чтобы самостоятельно создавать экземпляр wasm. Это позволяет
    // запускать инициализацию параллельно с другими асинхронными действиями.
    // Также pthread и wasm-воркеры идут через этот путь.
    if (Module['instantiateWasm']) {
        try {
            return Module['instantiateWasm'](info, receiveInstance);
        } catch (e) {
            err(`Module.instantiateWasm не удалось вызвать: ${e}`);
            return false;
        }
    }

    var result = instantiateSync(wasmBinaryFile, info);
    return receiveInstance(result[0]);
}

// Глобальные переменные для конверсий i64 (см. makeSetValue)
var tempDouble;
var tempI64;

// include: runtime_debug.js
function legacyModuleProp(prop, newName, incoming = true) {
    if (!Object.getOwnPropertyDescriptor(Module, prop)) {
        Object.defineProperty(Module, prop, {
            configurable: true,
            get() {
                let extra = incoming
                    ? ' (начальное значение можно задать в Module, но после старта берётся только локальная переменная)'
                    : '';
                abort(`\`Module.${prop}\` заменён на \`${newName}\`` + extra);
            }
        });
    }
}

function ignoredModuleProp(prop) {
    if (Object.getOwnPropertyDescriptor(Module, prop)) {
        abort(`\`Module.${prop}\` был передан, но \`${prop}\` не включён в INCOMING_MODULE_JS_API`);
    }
}

// Экспорты файловой системы по умолчанию
function isExportedByForceFilesystem(name) {
    return name === 'FS_createPath' ||
        name === 'FS_createDataFile' ||
        name === 'FS_createPreloadedFile' ||
        name === 'FS_unlink' ||
        name === 'addRunDependency' ||
        name === 'FS_createLazyFile' ||
        name === 'FS_createDevice' ||
        name === 'removeRunDependency';
}

function missingGlobal(sym, msg) {
    if (typeof globalThis !== 'undefined') {
        Object.defineProperty(globalThis, sym, {
            configurable: true,
            get() {
                warnOnce(`\`${sym}\` больше не определяется emscripten. ${msg}`);
                return undefined;
            }
        });
    }
}

missingGlobal('buffer', 'Используйте HEAP8.buffer или wasmMemory.buffer');
missingGlobal('asm', 'Используйте wasmExports вместо этого');

function missingLibrarySymbol(sym) {
    if (typeof globalThis !== 'undefined' && !Object.getOwnPropertyDescriptor(globalThis, sym)) {
        Object.defineProperty(globalThis, sym, {
            configurable: true,
            get() {
                var msg = `\`${sym}\` — это символ библиотеки и по умолчанию не включён; добавьте его в __deps вашей library.js или в DEFAULT_LIBRARY_FUNCS_TO_INCLUDE на командной строке`;
                var librarySymbol = sym;
                if (!librarySymbol.startsWith('_')) {
                    librarySymbol = '$' + sym;
                }
                msg += ` (например, -sDEFAULT_LIBRARY_FUNCS_TO_INCLUDE='${librarySymbol}')`;
                if (isExportedByForceFilesystem(sym)) {
                    msg += '. Или включите поддержку файловой системы (-sFORCE_FILESYSTEM), чтобы экспортировать его автоматически';
                }
                warnOnce(msg);
                return undefined;
            }
        });
    }
    unexportedRuntimeSymbol(sym);
}

function unexportedRuntimeSymbol(sym) {
    if (!Object.getOwnPropertyDescriptor(Module, sym)) {
        Object.defineProperty(Module, sym, {
            configurable: true,
            get() {
                var msg = `'${sym}' не экспортирован. Добавьте его в EXPORTED_RUNTIME_METHODS (см. FAQ по Emscripten)`;
                if (isExportedByForceFilesystem(sym)) {
                    msg += '. Или включите поддержку файловой системы (-sFORCE_FILESYSTEM), чтобы экспортировать его автоматически';
                }
                abort(msg);
            }
        });
    }
}

// Используется для отладки (XXXXX_DEBUG) и вывода сообщений
function dbg(...args) {
    console.warn(...args);
}

// end include: runtime_debug.js
// === Основная часть ===

var ASM_CONSTS = {
    68692: ($0, $1) => {
        if (typeof window !== 'undefined' && window.crypto && window.crypto.getRandomValues) {
            var randBuffer = new Uint8Array($1);
            window.crypto.getRandomValues(randBuffer);
            writeArrayToMemory(randBuffer, $0);
            return 0;
        }
        var cryptoMod;
        try {
            cryptoMod = require('crypto');
        } catch (error) {
            return -2;
        }
        try {
            writeArrayToMemory(cryptoMod.randomBytes($1), $0);
            return 0;
        } catch (error) {
            return -1;
        }
    }
};

// end include: preamble.js

/** @constructor */
function ExitStatus(status) {
    this.name = 'ExitStatus';
    this.message = `Программа завершилась с exit(${status})`;
    this.status = status;
}
var callRuntimeCallbacks = (callbacks) => {
    while (callbacks.length > 0) {
        // Передаём объект Module первым аргументом
        callbacks.shift()(Module);
    }
};

/**
 * Читает значение из памяти по указателю
 * @param {number} ptr
 * @param {string} type
 */
function getValue(ptr, type = 'i8') {
    if (type.endsWith('*')) type = '*';
    switch (type) {
        case 'i1':
        case 'i8':
            return HEAP8[ptr];
        case 'i16':
            return HEAP16[((ptr) >> 1)];
        case 'i32':
            return HEAP32[((ptr) >> 2)];
        case 'i64':
            abort('Для getValue(i64) используйте WASM_BIGINT');
        case 'float':
            return HEAPF32[((ptr) >> 2)];
        case 'double':
            return HEAPF64[((ptr) >> 3)];
        case '*':
            return HEAPU32[((ptr) >> 2)];
        default:
            abort(`Недопустимый тип в getValue: ${type}`);
    }
}

var noExitRuntime = Module['noExitRuntime'] || true;

// Преобразует указатель в строку hex
var ptrToString = (ptr) => {
    assert(typeof ptr === 'number');
    ptr >>>= 0;
    return '0x' + ptr.toString(16).padStart(8, '0');
};

/**
 * Записывает значение в память по указателю
 * @param {number} ptr
 * @param {number} value
 * @param {string} type
 */
function setValue(ptr, value, type = 'i8') {
    if (type.endsWith('*')) type = '*';
    switch (type) {
        case 'i1':
        case 'i8':
            HEAP8[ptr] = value;
            break;
        case 'i16':
            HEAP16[((ptr) >> 1)] = value;
            break;
        case 'i32':
            HEAP32[((ptr) >> 2)] = value;
            break;
        case 'i64':
            abort('Для setValue(i64) используйте WASM_BIGINT');
        case 'float':
            HEAPF32[((ptr) >> 2)] = value;
            break;
        case 'double':
            HEAPF64[((ptr) >> 3)] = value;
            break;
        case '*':
            HEAPU32[((ptr) >> 2)] = value;
            break;
        default:
            abort(`Недопустимый тип в setValue: ${type}`);
    }
}

// Восстановление стека
var stackRestore = (val) => __emscripten_stack_restore(val);

// Сохранение текущего указателя стека
var stackSave = () => _emscripten_stack_get_current();

// Показывает предупреждение только один раз
var warnOnce = (text) => {
    warnOnce.shown ||= {};
    if (!warnOnce.shown[text]) {
        warnOnce.shown[text] = 1;
        if (ENVIRONMENT_IS_NODE) text = 'warning: ' + text;
        err(text);
    }
};

// Быстрое копирование памяти
var __emscripten_memcpy_js = (dest, src, num) => HEAPU8.copyWithin(dest, src, src + num);

var readEmAsmArgsArray = [];
var readEmAsmArgs = (sigPtr, buf) => {
    assert(Array.isArray(readEmAsmArgsArray));
    assert(buf % 16 == 0);
    readEmAsmArgsArray.length = 0;
    var ch;
    while (ch = HEAPU8[sigPtr++]) {
        var chr = String.fromCharCode(ch);
        var validChars = ['d', 'f', 'i', 'p'];
        assert(validChars.includes(chr), `Недопустимый символ ${ch} ("${chr}") в readEmAsmArgs! Используйте только [${validChars}], и не указывайте "v" для void возвращаемого типа.`);
        var wide = (ch != 105) && (ch != 112);
        buf += wide && (buf % 8) ? 4 : 0;
        readEmAsmArgsArray.push(
            ch == 112 ? HEAPU32[((buf) >> 2)] :
                ch == 105 ? HEAP32[((buf) >> 2)] :
                    HEAPF64[((buf) >> 3)]
        );
        buf += wide ? 8 : 4;
    }
    return readEmAsmArgsArray;
};

var runEmAsmFunction = (code, sigPtr, argbuf) => {
    var args = readEmAsmArgs(sigPtr, argbuf);
    assert(ASM_CONSTS.hasOwnProperty(code), `Не найден EM_ASM константа по адресу ${code}. Загрузка wasm, вероятно, не совпадает с JS.`);
    return ASM_CONSTS[code](...args);
};

var _emscripten_asm_const_int = (code, sigPtr, argbuf) => {
    return runEmAsmFunction(code, sigPtr, argbuf);
};

var getHeapMax = () => HEAPU8.length;

var abortOnCannotGrowMemory = (requestedSize) => {
    abort(`Невозможно увеличить память до ${requestedSize} байт (OOM). Либо (1) соберите с -sINITIAL_MEMORY=X с X больше текущего ${HEAP8.length}, (2) соберите с -sALLOW_MEMORY_GROWTH для увеличения в рантайме, или (3) чтобы malloc возвращал NULL (0), а не завершал работу, соберите с -sABORTING_MALLOC=0`);
};

var _emscripten_resize_heap = (requestedSize) => {
    var oldSize = HEAPU8.length;
    requestedSize >>>= 0;
    abortOnCannotGrowMemory(requestedSize);
};

var runtimeKeepaliveCounter = 0;
var keepRuntimeAlive = () => noExitRuntime || runtimeKeepaliveCounter > 0;

var _proc_exit = (code) => {
    EXITSTATUS = code;
    if (!keepRuntimeAlive()) {
        Module['onExit']?.(code);
        ABORT = true;
    }
    quit_(code, new ExitStatus(code));
};

/** @param {boolean|number=} implicit */
var exitJS = (status, implicit) => {
    EXITSTATUS = status;

    checkUnflushedContent();

    if (keepRuntimeAlive() && !implicit) {
        var msg = `Программа завершена (код ${status}), но keepRuntimeAlive() включён (счётчик=${runtimeKeepaliveCounter}) из-за асинхронной операции, поэтому выполнение останавливается, но рантайм не завершается и дальнейшее асинхронное выполнение возможно (для принудительного завершения используйте emscripten_force_exit)`;
        err(msg);
    }

    _proc_exit(status);
};

var _exit = exitJS;

// Записывает массив байт в память по указанному адресу
var writeArrayToMemory = (array, buffer) => {
    assert(array.length >= 0, 'writeArrayToMemory: массив должен иметь length (это должен быть Array или TypedArray)');
    HEAP8.set(array, buffer);
};

function checkIncomingModuleAPI() {
    ignoredModuleProp('fetchSettings');
}

var wasmImports = {
    /** @export */
    _emscripten_memcpy_js: __emscripten_memcpy_js,
    /** @export */
    emscripten_asm_const_int: _emscripten_asm_const_int,
    /** @export */
    emscripten_resize_heap: _emscripten_resize_heap,
    /** @export */
    exit: _exit
};

// Создаём экземпляр wasm
var wasmExports = createWasm();

var ___wasm_call_ctors = createExportWrapper('__wasm_call_ctors', 0);
var _mem_alloc = Module['_mem_alloc'] = createExportWrapper('mem_alloc', 1);
var _malloc = Module['_malloc'] = createExportWrapper('malloc', 1);
var _mem_alloc_long_long = Module['_mem_alloc_long_long'] = createExportWrapper('mem_alloc_long_long', 1);
var _mem_free = Module['_mem_free'] = createExportWrapper('mem_free', 2);
var _free = Module['_free'] = createExportWrapper('free', 1);
var _dp_sign_keypair_seed = Module['_dp_sign_keypair_seed'] = createExportWrapper('dp_sign_keypair_seed', 3);
var _dp_sign_keypair = Module['_dp_sign_keypair'] = createExportWrapper('dp_sign_keypair', 2);
var _dp_sign = Module['_dp_sign'] = createExportWrapper('dp_sign', 5);
var _dp_sign_verify = Module['_dp_sign_verify'] = createExportWrapper('dp_sign_verify', 5);
var _dp_sign_seedexpander = Module['_dp_sign_seedexpander'] = createExportWrapper('dp_sign_seedexpander', 2);
var _dp_randombytes = Module['_dp_randombytes'] = createExportWrapper('dp_randombytes', 2);
var _dp_seedexpander_wrapper = Module['_dp_seedexpander_wrapper'] = createExportWrapper('dp_seedexpander_wrapper', 4);
var _fflush = createExportWrapper('fflush', 1);

var _emscripten_stack_init = wasmExports['emscripten_stack_init']
var _emscripten_stack_get_free = wasmExports['emscripten_stack_get_free']
var _emscripten_stack_get_base = wasmExports['emscripten_stack_get_base']
var _emscripten_stack_get_end = wasmExports['emscripten_stack_get_end']
var __emscripten_stack_restore = wasmExports['_emscripten_stack_restore']
var __emscripten_stack_alloc = wasmExports['_emscripten_stack_alloc']
var _emscripten_stack_get_current = wasmExports['emscripten_stack_get_current']

// include: postamble.js
// === Автоматически сгенерированная часть завершения ===

Module['getValue'] = getValue;
Module['writeArrayToMemory'] = writeArrayToMemory;

// Символы библиотеки, которые отсутствуют (покажем предупреждения при доступе)
var missingLibrarySymbols = [
    'writeI53ToI64',
    'writeI53ToI64Clamped',
    'writeI53ToI64Signaling',
    'writeI53ToU64Clamped',
    'writeI53ToU64Signaling',
    'readI53FromI64',
    'readI53FromU64',
    'convertI32PairToI53',
    'convertI32PairToI53Checked',
    'convertU32PairToI53',
    'stackAlloc',
    'getTempRet0',
    'setTempRet0',
    'zeroMemory',
    'growMemory',
    'isLeapYear',
    'ydayFromDate',
    'arraySum',
    'addDays',
    'inetPton4',
    'inetNtop4',
    'inetPton6',
    'inetNtop6',
    'readSockaddr',
    'writeSockaddr',
    'initRandomFill',
    'randomFill',
    'emscriptenLog',
    'runMainThreadEmAsm',
    'jstoi_q',
    'getExecutableName',
    'listenOnce',
    'autoResumeAudioContext',
    'dynCallLegacy',
    'getDynCaller',
    'dynCall',
    'handleException',
    'runtimeKeepalivePush',
    'runtimeKeepalivePop',
    'callUserCallback',
    'maybeExit',
    'asmjsMangle',
    'asyncLoad',
    'alignMemory',
    'mmapAlloc',
    'HandleAllocator',
    'getNativeTypeSize',
    'STACK_SIZE',
    'STACK_ALIGN',
    'POINTER_SIZE',
    'ASSERTIONS',
    'getCFunc',
    'ccall',
    'cwrap',
    'uleb128Encode',
    'sigToWasmTypes',
    'generateFuncType',
    'convertJsFunctionToWasm',
    'getEmptyTableSlot',
    'updateTableMap',
    'getFunctionAddress',
    'addFunction',
    'removeFunction',
    'reallyNegative',
    'unSign',
    'strLen',
    'reSign',
    'formatString',
    'UTF8ArrayToString',
    'UTF8ToString',
    'stringToUTF8Array',
    'stringToUTF8',
    'lengthBytesUTF8',
    'intArrayFromString',
    'intArrayToString',
    'AsciiToString',
    'stringToAscii',
    'UTF16ToString',
    'stringToUTF16',
    'lengthBytesUTF16',
    'UTF32ToString',
    'stringToUTF32',
    'lengthBytesUTF32',
    'stringToNewUTF8',
    'stringToUTF8OnStack',
    'registerKeyEventCallback',
    'maybeCStringToJsString',
    'findEventTarget',
    'getBoundingClientRect',
    'fillMouseEventData',
    'registerMouseEventCallback',
    'registerWheelEventCallback',
    'registerUiEventCallback',
    'registerFocusEventCallback',
    'fillDeviceOrientationEventData',
    'registerDeviceOrientationEventCallback',
    'fillDeviceMotionEventData',
    'registerDeviceMotionEventCallback',
    'screenOrientation',
    'fillOrientationChangeEventData',
    'registerOrientationChangeEventCallback',
    'fillFullscreenChangeEventData',
    'registerFullscreenChangeEventCallback',
    'JSEvents_requestFullscreen',
    'JSEvents_resizeCanvasForFullscreen',
    'registerRestoreOldStyle',
    'hideEverythingExceptGivenElement',
    'restoreHiddenElements',
    'setLetterbox',
    'softFullscreenResizeWebGLRenderTarget',
    'doRequestFullscreen',
    'fillPointerlockChangeEventData',
    'registerPointerlockChangeEventCallback',
    'registerPointerlockErrorEventCallback',
    'requestPointerLock',
    'fillVisibilityChangeEventData',
    'registerVisibilityChangeEventCallback',
    'registerTouchEventCallback',
    'fillGamepadEventData',
    'registerGamepadEventCallback',
    'registerBeforeUnloadEventCallback',
    'fillBatteryEventData',
    'battery',
    'registerBatteryEventCallback',
    'setCanvasElementSize',
    'getCanvasElementSize',
    'jsStackTrace',
    'getCallstack',
    'convertPCtoSourceLocation',
    'getEnvStrings',
    'checkWasiClock',
    'flush_NO_FILESYSTEM',
    'wasiRightsToMuslOFlags',
    'wasiOFlagsToMuslOFlags',
    'createDyncallWrapper',
    'safeSetTimeout',
    'setImmediateWrapped',
    'clearImmediateWrapped',
    'polyfillSetImmediate',
    'getPromise',
    'makePromise',
    'idsToPromises',
    'makePromiseCallback',
    'ExceptionInfo',
    'findMatchingCatch',
    'Browser_asyncPrepareDataCounter',
    'setMainLoop',
    'getSocketFromFD',
    'getSocketAddress',
    'FS_createPreloadedFile',
    'FS_modeStringToFlags',
    'FS_getMode',
    'FS_stdin_getChar',
    'FS_createDataFile',
    'FS_unlink',
    'FS_mkdirTree',
    '_setNetworkCallback',
    'heapObjectForWebGLType',
    'toTypedArrayIndex',
    'webgl_enable_ANGLE_instanced_arrays',
    'webgl_enable_OES_vertex_array_object',
    'webgl_enable_WEBGL_draw_buffers',
    'webgl_enable_WEBGL_multi_draw',
    'emscriptenWebGLGet',
    'computeUnpackAlignedImageSize',
    'colorChannelsInGlTextureFormat',
    'emscriptenWebGLGetTexPixelData',
    'emscriptenWebGLGetUniform',
    'webglGetUniformLocation',
    'webglPrepareUniformLocationsBeforeFirstUse',
    'webglGetLeftBracePos',
    'emscriptenWebGLGetVertexAttrib',
    '__glGetActiveAttribOrUniform',
    'writeGLArray',
    'registerWebGlEventCallback',
    'runAndAbortIfError',
    'ALLOC_NORMAL',
    'ALLOC_STACK',
    'allocate',
    'writeStringToMemory',
    'writeAsciiToMemory',
    'setErrNo',
    'demangle',
    'stackTrace',
];

// Для каждого отсутствующего символа создаём предупреждение при доступе
missingLibrarySymbols.forEach(missingLibrarySymbol)
var unexportedSymbols = [
    'run',
    'addOnPreRun',
    'addOnInit',
    'addOnPreMain',
    'addOnExit',
    'addOnPostRun',
    'addRunDependency',
    'removeRunDependency',
    'FS_createFolder',
    'FS_createPath',
    'FS_createLazyFile',
    'FS_createLink',
    'FS_createDevice',
    'FS_readFile',
    'out',
    'err',
    'callMain',
    'abort',
    'wasmMemory',
    'wasmExports',
    'writeStackCookie',
    'checkStackCookie',
    'intArrayFromBase64',
    'tryParseAsDataURI',
    'stackSave',
    'stackRestore',
    'ptrToString',
    'exitJS',
    'getHeapMax',
    'abortOnCannotGrowMemory',
    'ENV',
    'MONTH_DAYS_REGULAR',
    'MONTH_DAYS_LEAP',
    'MONTH_DAYS_REGULAR_CUMULATIVE',
    'MONTH_DAYS_LEAP_CUMULATIVE',
    'ERRNO_CODES',
    'ERRNO_MESSAGES',
    'DNS',
    'Protocols',
    'Sockets',
    'timers',
    'warnOnce',
    'readEmAsmArgsArray',
    'readEmAsmArgs',
    'runEmAsmFunction',
    'jstoi_s',
    'keepRuntimeAlive',
    'wasmTable',
    'noExitRuntime',
    'freeTableIndexes',
    'functionsInTableMap',
    'setValue',
    'PATH',
    'PATH_FS',
    'UTF8Decoder',
    'UTF16Decoder',
    'JSEvents',
    'specialHTMLTargets',
    'findCanvasEventTarget',
    'currentFullscreenStrategy',
    'restoreOldWindowedStyle',
    'UNWIND_CACHE',
    'ExitStatus',
    'promiseMap',
    'uncaughtExceptionCount',
    'exceptionLast',
    'exceptionCaught',
    'Browser',
    'getPreloadedImageData__data',
    'wget',
    'SYSCALLS',
    'preloadPlugins',
    'FS_stdin_getChar_buffer',
    'FS',
    'MEMFS',
    'TTY',
    'PIPEFS',
    'SOCKFS',
    'tempFixedLengthArray',
    'miniTempWebGLFloatBuffers',
    'miniTempWebGLIntBuffers',
    'GL',
    'AL',
    'GLUT',
    'EGL',
    'GLEW',
    'IDBStore',
    'SDL',
    'SDL_gfx',
    'allocateUTF8',
    'allocateUTF8OnStack',
];
unexportedSymbols.forEach(unexportedRuntimeSymbol);

var calledRun;

// Функция вызывается, когда выполнены все зависимости
dependenciesFulfilled = function runCaller() {
    if (!calledRun) run();
    if (!calledRun) dependenciesFulfilled = runCaller; // если снова добавят зависимости, повторим позже
};

function stackCheckInit() {
    // Обычно вызывается автоматически внутри __wasm_call_ctors,
    // но здесь нужен до вызова конструкторов
    _emscripten_stack_init();
    // TODO(sbc): Перенести writeStackCookie в нативный код
    writeStackCookie();
}

function run() {

    if (runDependencies > 0) {
        return;
    }

    stackCheckInit();

    preRun();

    // preRun добавил зависимости, запуск откладываем
    if (runDependencies > 0) {
        return;
    }

    function doRun() {
        // Если run уже был вызван, ничего не делаем
        if (calledRun) return;
        calledRun = true;
        Module['calledRun'] = true;

        if (ABORT) return;

        initRuntime();

        if (Module['onRuntimeInitialized']) Module['onRuntimeInitialized']();

        assert(!Module['_main'], 'Компилировано без main, но он есть. Если вы добавили его из JS, используйте Module["onRuntimeInitialized"]');

        postRun();
    }

    if (Module['setStatus']) {
        Module['setStatus']('Запуск...');
        setTimeout(function () {
            setTimeout(function () {
                Module['setStatus']('');
            }, 1);
            doRun();
        }, 1);
    } else {
        doRun();
    }
    checkStackCookie();
}

function checkUnflushedContent() {
    // Настройки компилятора не позволяют выходить из рантайма,
    // поэтому сброс потоков невозможен. Но в режиме ASSERTIONS
    // проверяем, есть ли что-то несброшенное, и предупреждаем.
    // Обычно flush() не включается, но в ASSERTIONS мы его оставляем.
    var oldOut = out;
    var oldErr = err;
    var has = false;
    out = err = (x) => {
        has = true;
    }
    try {
        _fflush(0);
    } catch (e) {
    }
    out = oldOut;
    err = oldErr;
    if (has) {
        warnOnce('Потоки stdio содержали данные, которые не были сброшены. Установите EXIT_RUNTIME=1 (см. FAQ Emscripten) или убедитесь, что добавляете перевод строки в printf и др.');
        warnOnce('(Это также может быть из-за отсутствия поддержки файловой системы - попробуйте собрать с -sFORCE_FILESYSTEM)');
    }
}

// Если задан Module['preInit'], вызываем его
if (Module['preInit']) {
    if (typeof Module['preInit'] == 'function') Module['preInit'] = [Module['preInit']];
    while (Module['preInit'].length > 0) {
        Module['preInit'].pop()();
    }
}

// Запуск приложения
run();

// end include: postamble.js
